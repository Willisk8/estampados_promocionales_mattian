-- ============================================================
-- 055_quote_simple_idempotency.sql
--
-- Corrige QA-QUOTE-004: doble clic/retry de
-- fn_consola_crear_cotizacion_simple emitia cotizaciones duplicadas.
--
-- La clave de idempotencia es opcional para compatibilidad; cuando se envia,
-- el mismo usuario autenticado recibe la cotizacion existente en un retry.
-- ============================================================

ALTER TABLE cotizacion
    ADD COLUMN IF NOT EXISTS idempotency_key TEXT;

CREATE UNIQUE INDEX IF NOT EXISTS uq_cotizacion_creador_idempotency_key
    ON cotizacion (creada_por, idempotency_key)
    WHERE idempotency_key IS NOT NULL;

COMMENT ON COLUMN cotizacion.idempotency_key IS
    'Clave opcional de idempotencia del cliente/UI. Reintentos del mismo usuario devuelven la cotizacion existente.';

DROP FUNCTION IF EXISTS fn_consola_crear_cotizacion_simple(UUID, UUID, UUID, INTEGER, TEXT, TEXT, UUID, JSONB);
...
COMMENT ON FUNCTION fn_consola_crear_cotizacion_simple(UUID, UUID, UUID, INTEGER, TEXT, TEXT, UUID, JSONB, TEXT) IS
    'Crea cotizacion simple por tarifa publicada. Desde 055 acepta p_idempotency_key opcional y devuelve la cotizacion existente en retries del mismo usuario.';
