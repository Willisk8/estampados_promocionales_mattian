-- ============================================================
-- 054_fix_quote_component_rounding_and_marking_sources.sql
--
-- Corrige hallazgos QA-038-001/002/003 sobre el motor de cotizacion por
-- componentes vigente tras 049:
--   1. precio_resultante debe aplicar la regla comercial de redondeo.
--   2. si se especifica tecnica, MARCACION debe salir de snapshots curados
--      de precio_tecnica_marcacion_snapshot, no de costo_producto.
--   3. p_numero_preparaciones debe validarse y materializar componente
--      PREPARACION cuando haya preparaciones.
--
-- No edita migraciones ya aplicadas. Sobrescribe la funcion vigente de forma
-- incremental y agrega el costo de preparacion como configuracion explicita en
-- producto_tecnica.
-- ============================================================

ALTER TABLE producto_tecnica
    ADD COLUMN IF NOT EXISTS costo_preparacion NUMERIC(12,2) NOT NULL DEFAULT 0
        CHECK (costo_preparacion >= 0),
    ADD COLUMN IF NOT EXISTS moneda_preparacion TEXT NOT NULL DEFAULT 'COP';

COMMENT ON COLUMN producto_tecnica.costo_preparacion IS
    'Costo fijo por preparacion/setup para esta combinacion producto/tecnica. Se multiplica por p_numero_preparaciones.';

COMMENT ON COLUMN producto_tecnica.moneda_preparacion IS
    'Moneda del costo_preparacion; debe coincidir con la moneda de la cotizacion para usarse automatico.';

ALTER TABLE cotizacion_componente
    DROP CONSTRAINT IF EXISTS cotizacion_componente_source_type_check,
    ADD CONSTRAINT cotizacion_componente_source_type_check
...
BEGIN
    IF v_rol IS DISTINCT FROM 'ADMIN' THEN
        RETURN QUERY SELECT
            ...
            'PASS_THROUGH'::TEXT, 0::NUMERIC, 0::NUMERIC,
    ...
    IF p_id_tecnica IS NOT NULL THEN
        SELECT
          ...
          JOIN curacion_precio_tecnica_marcacion c
            ON c.id_snapshot = pts.id_snapshot
         WHERE pts.id_tecnica = p_id_tecnica
           ...
           -- MVP seguro: solo unidad puede convertirse a costo unitario sin
           -- dimensiones/area. Metro lineal, hoja o A4 requieren entradas
           -- adicionales antes de calcular automatico.
    ...
    raw_components AS (
        ...
        SELECT
            'MARCACION',
            'Marcacion/personalizacion desde snapshot curado',
            p_cantidad::NUMERIC,
            v_marking_snapshot.price_value::NUMERIC,
            (v_marking_snapshot.price_value * p_cantidad)::NUMERIC,
            'PRECIO_TECNICA_SNAPSHOT',
            v_marking_snapshot.id_snapshot,
            jsonb_build_object(
                'id_tecnica', p_id_tecnica,
                'producto_tecnica', v_producto_tecnica.id_producto_tecnica,
                'billing_unit', v_marking_snapshot.billing_unit,
                'formula_code', v_marking_snapshot.formula_code,
                'quantity_min', v_marking_snapshot.quantity_min,
                'quantity_max', v_marking_snapshot.quantity_max
            )
        ...
        SELECT
            'MARCACION',
            'Marcacion/personalizacion desde costo_producto',
            p_cantidad::NUMERIC,
            ...
            v_cost.id_costo,
            jsonb_build_object('id_tecnica', p_id_tecnica)
        WHERE p_id_tecnica IS NULL
          AND v_cost.costo_personalizacion > 0

        UNION ALL
        SELECT
            'PREPARACION',
            'Preparacion/setup',
            p_numero_preparaciones::NUMERIC,
            v_producto_tecnica.costo_preparacion::NUMERIC,
            (v_producto_tecnica.costo_preparacion * p_numero_preparaciones)::NUMERIC,
            'PRODUCTO_TECNICA',
            v_producto_tecnica.id_producto_tecnica,
            jsonb_build_object(
                'id_tecnica', p_id_tecnica,
                'moneda_preparacion', v_producto_tecnica.moneda_preparacion
            )
        WHERE p_id_tecnica IS NOT NULL
          AND p_numero_preparaciones > 0
...
COMMENT ON FUNCTION fn_calculate_quote_components(UUID, UUID, INTEGER, UUID, INTEGER, NUMERIC, TEXT, TIMESTAMPTZ, TEXT) IS
    'Corregido en 054: solo ADMIN; aplica redondeo comercial; usa snapshot curado para marcacion explicita; valida y materializa preparaciones.';
