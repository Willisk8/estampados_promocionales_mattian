-- ============================================================
-- 057_fix_quote_components_without_marking_technique.sql
--
-- Corrige regresion detectada al probar 054: cuando p_id_tecnica es NULL,
-- v_marking_snapshot nunca recibia estructura de RECORD. Aunque la rama de
-- snapshot no se usa, PostgreSQL valida las referencias dentro del UNION y
-- aborta con "record is not assigned yet".
--
-- Mantiene el contrato de 054/056 y solo inicializa el RECORD con columnas
-- nulas antes de resolver una tecnica real.
-- ============================================================
...
COMMENT ON FUNCTION fn_calculate_quote_components(UUID, UUID, INTEGER, UUID, INTEGER, NUMERIC, TEXT, TIMESTAMPTZ, TEXT) IS
    'Corregido hasta 057: solo ADMIN; aplica redondeo comercial; usa snapshot curado para marcacion explicita; valida preparaciones; soporta cotizaciones sin tecnica.';
